...
{
	while (!(*has_ended) && ((*todos)[TO_PROD_OFFSET]-- > 0)) {
...
{
	while (!(*has_ended) && ((*todos)[TO_CONS_OFFSET]-- > 0)) {
...
int main(int argc, char *argv[])
{
  ...
  *has_ended = false; // initalise value of sigint check
  ...
  FILE *log_csv = fopen("./processes_timings.csv", "w+");
  fprintf(log_csv, "Iterations, Duration\n");
  for (int i = START_NUM_ITER; i < END_NUM_ITER; i += SAMPLE_DISTANCE) {
    // initalise value of todos
    (*todos)[TO_PROD_OFFSET] = i; 
    (*todos)[TO_CONS_OFFSET] = i;
  ...
  printf("Detached todos shared memory.\n");
